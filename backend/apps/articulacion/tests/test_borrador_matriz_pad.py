...
Adaptación a main (integracion-s2): main no tiene ``cuenta_con_financiamiento``
(ResultadoPAD/ProductoPAD), ``politica``, ni ``presupuesto_total``/
...
class BorradorMatrizPADAPITest(TestCase):
    ...
    def test_flujo_completo_coleccion_resultados(self):
        ...
        # 6. Matriz B en vivo: 2 filas (1 por resultado, SIN desglosar
        # productos — Matriz B oficial, guía 4.5.2). La rama s2 tiene el
        # mismo contrato en su servicio; su test con 5 filas no coincidía.
        response = self.client.get(
        ...
        # TODO-articulacion-s2: main no persiste presupuesto_total/
        # presupuesto_anual (ni cuenta_con_financiamiento) en los modelos;
        # las filas materializadas los devuelven degradados ('').
        self.assertEqual(response.data[0]['presupuesto_total'], '')
        self.assertEqual(response.data[1]['nivel'], 'PRODUCTO_PAD')
        self.assertEqual(response.data[1]['presupuesto_2026'], '')
        self.assertEqual(
            response.data[3]['codigo_resultado_pad'], '031001.05.2',
        )

        # 11. Matriz B desde modelos: 2 filas (nivel resultado, sin productos)
        response = self.client.get(
            f'/api/v1/articulacion/borradores-matriz-pad/{borrador_id}/matriz_b/'
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_materializar_preserva_valores_falsy(self):
        """linea_base=0 / meta_2030=0 / programacion_fisica=0 no se pierden."""
        borrador_id = self._crear_borrador_con_cabecera()
        coleccion = [{
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK, response.data)
        # El indicador de resultado se crea degradado (sin FK resultado_pad:
        # main no tiene la FK); el de producto queda ligado al producto.
        ind_producto = IndicadorCadena.objects.get(nivel_indicador='PRODUCTO_PAD')
        self.assertEqual(ind_producto.linea_base, 0)
        self.assertEqual(ind_producto.meta_2030, 0)
        self.assertEqual(ind_producto.programacion_fisica, {'2026': 0})
        # TODO-articulacion-s2: main no persiste presupuesto_total/
        # presupuesto_anual en IndicadorCadena; degradados.
        self.assertEqual(
