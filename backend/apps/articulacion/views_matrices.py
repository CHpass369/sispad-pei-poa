    ...
    def catalogos_articulacion(self, request):
        ...
        ejes = [
            {'codigo': e.codigo, 'denominacion': e.denominacion, 'id': str(e.id),
             'objetivo_impacto': e.objetivo_impacto or ''}
        ...
        componentes = [
            {'codigo': c.codigo, 'denominacion': c.denominacion, 'id': str(c.id),
             ...
             'objetivo_efecto': c.objetivo_efecto or ''}
        ...
        lineamientos = [
            ...
            for l in _by_gestion(LineamientoPAD.objects.select_related('componente').all()).order_by('codigo')
        ]
        ...
        if not lineamientos:
            lineamientos = [
                {'codigo': l.codigo, 'denominacion': l.denominacion, 'id': str(l.id),
                 'componente_codigo': getattr(getattr(l, 'componente', None), 'codigo', '')}
                for l in LineamientoPAD.objects.select_related('componente').filter(
                    version_catalogo__gestion=2026
                ).order_by('codigo')
            ]
        unidades = [
