  ...
  template: `
    <div class="wizard">
      <div class="wizard-header">
        ...
        @if (unidad) {
          <p class="text-secondary">{{ unidad.nombre }}</p>
        }
      </div>
            ...
            <form [formGroup]="form.controls.articulacion">
              ...
              <div class="field">
                ...
                <select formControlName="accion_mediano" class="form-control">
                  <option value="">Seleccione...</option>
                  @for (amp of accionesMediano; track amp) {
                    <option [value]="amp.id">{{ amp.codigo }} - {{ amp.nombre }}</option>
                  }
                </select>
              </div>
              <div class="field">
                ...
                <div class="row">
                  ...
                  <div class="field third">
                    ...
                    <select formControlName="unidad_medida" class="form-control">
                      @for (u of unidadesMedida; track u) {
                        <option [value]="u.id">{{ u.codigo }} - {{ u.denominacion }}</option>
                      }
                    </select>
                  </div>
                </div>
            ...
            <div>
              ...
              <form [formGroup]="operacionesFormGroup">
                <div class="operations-list">
                  @for (op of operacionesForm.controls; track op; let i = $index) {
                    <div class="operation-item card">
                      <div [formGroupName]="i">
                        ...
                        <div class="field">
                          ...
                          <select formControlName="objeto_gasto" class="form-control">
                            @for (og of objetosGasto; track og) {
                              <option [value]="og.id">{{ og.codigo }} - {{ og.denominacion }}</option>
                            }
                          </select>
                        </div>
                      ...
                      @if (operacionesForm.length > 1) {
                        <button class="btn btn-outline btn-sm" (click)="eliminarOperacion(i)">
                          Eliminar
                        </button>
                      }
                    </div>
